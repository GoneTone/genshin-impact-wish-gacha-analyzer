<template>
  ...
  <div class="progress mb-0" v-if="!isHome">
  ...
  <p class="mb-4 text-bd6932" v-if="!isHome">距離保底還剩下 <kbd>{{ guaranteedCountComputed.toString() }}</kbd> 抽！</p>
</template>
...
export default {
  ...
  methods: {
    getGuaranteedCountComputed () {
      const guaranteedCount = this.$store.getters.configs.game.guaranteedCount
