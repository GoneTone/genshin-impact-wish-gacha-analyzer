  ...
  <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
    ...
    <ul class="navbar-nav ml-auto">
      <li class="nav-item dropdown no-arrow mx-1">
        <a class="nav-link" @click="openExternal(this.$store.getters.configs.team.facebookUrl);">
          <i class="fab fa-facebook-f fa-fw"></i>
        </a>
      </li>

      <li class="nav-item dropdown no-arrow mx-1">
        <a class="nav-link" @click="openExternal(this.$store.getters.configs.team.discordUrl);">
      ...
      <li class="nav-item dropdown no-arrow">
        <a class="nav-link" @click="openExternal(this.$store.getters.configs.team.websiteUrl);">
          <span class="mr-2 text-gray-600 small">{{ this.$store.getters.configs.team.name }}</span>
        </a>
      </li>
    </ul>
  </nav>
