...
4. **`unifyDataLanguage()`（「立即轉換」按鈕）**：讀 `dataLanguageProvider`，`null` 回零結果；**立刻 emit `ConvertingDataLanguage` 進度狀態**（點擊當下即彈進度 dialog，文案「正在轉換資料語言」——不可重用 `Preparing`，那是 MITM 擷取 URL 用語「正在準備資料來源」）；**逐帳號** `convert → save`，**hints 跨帳號累積後一次 `setSearches`**（逐筆 `setSearch` 會各自整份重寫 `hoyowiki_index.json`＋emit，數百~數千筆 record 會 O(n) 磁碟寫入嚴重拖慢，故必須批次）；聚合 result，單帳號失敗吞例外 warning 續跑；刷新 state；最後 best-effort 跑既有 `_fetchHoYoWiki` 補抓目標語言的 icon／詳情（`pageByLang[target]`，讓角色說明與 gallery 頁籤即時可看），結尾主動清進度。**首次轉某語言**會抓該語言全部 `entry_page`（一次性、會快取，期間 emit `FetchingHoYoWiki` 進度）；**切回已抓過的語言** `needRefetchEntry` 全 false、近乎瞬間。
...
- **進度／結果 UI 生命週期**（對應姐妹 D）：
  - 進度：按下立刻 emit `ConvertingDataLanguage` → 既有進度 dialog 即時彈出（涵蓋「轉換＋catalog 抓取」期間）；prewarm 階段轉為既有 `FetchingHoYoWiki` 進度；因 unify 無 `UpdateCompleted` 終止狀態，結尾**主動清進度**關閉，避免卡死。切回已快取語言時轉換為純本地 remap、prewarm 無工作，dialog 一閃即逝。
...
- 「立即轉換」→ `unifyDataLanguage` → emit `ConvertingDataLanguage` → 逐帳號 `convert`（catalog 驅動）→ `save` → 跨帳號累積 hints 後一次 `setSearches` → 刷新 → best-effort `_fetchHoYoWiki` 補目標語言 icon／詳情 → 清進度 → 結果框。
