...
- **呈現**：走與既有「更新物品資料」相同的進度管線（`app_shell` 既有的 `ref.listen` 進度對話框機制）——即時顯示取得物品資料／下載圖片進度，結束顯示物品資料更新摘要。完成狀態**帶物品資料摘要、不帶匯入摘要**（避免顯示成手動匯入的結果文案）；**沒有實際補到任何東西時全程靜默、不彈完成訊息**。複用既有 i18n，無新字串。
